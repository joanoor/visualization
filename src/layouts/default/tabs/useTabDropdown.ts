import type { DropDownStruct, TabContentProps } from '@/types'
import { MenuEventEnum } from '@/types'
import { useMultipleTabStore } from '@/store'
import { RouteLocationNormalized, useRouter } from 'vue-router'
import useMultiTabs from '@/hooks/web/useMultiTabs'
...
export function useTabDropdown(
  ...
  const getDropMenuList = computed(() => {
    ...
    const dropMenuList: DropDownStruct[] = [
      {
  ...
  function handleMenuEvent(menu: DropDownStruct): void {
    const { event } = menu
    switch (event) {
