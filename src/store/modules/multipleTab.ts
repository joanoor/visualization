...
import { PAGE_NOT_FOUND_ROUTE, REDIRECT_ROUTE } from '@/router/baseRoutes'
import { getRawRoute } from '@/utils'
import { useAppStore } from '@/store'
import { useRedo } from '@/hooks/web/usePage'
import { MULTIPLE_TABS_KEY } from '@/settings/cacheSetting'
...
export const useMultipleTabStore = defineStore({
  ...
  state: (): MultipleTabState => {
    ...
    const cacheTab = useApp.getProjectConfig.multiTabsSetting.cache
    return {
  ...
  actions: {
    ...
    goToPage(router: Router) {
      ...
      let toPath = permissionStore.defaultRouter
    ...
    async addTab(route: RouteLocationNormalized) {
      ...
      this.updateCacheTab()
      this.tabList && Persistent.setLocal(MULTIPLE_TABS_KEY, this.tabList)
    },
    ...
    async closeTab(tab: RouteLocationNormalized, router: Router) {
      ...
      if (index === 0) {
        ...
        if (this.tabList.length === 1) {
          const permissionStore = usePermissionStore()
          toTarget = permissionStore.defaultRouter
        } else {
